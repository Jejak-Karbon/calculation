...
func (h *handler) Get(c echo.Context) error {
	...
	result, err := h.service.Find(c.Request().Context(), payload, uint(payloadToken.(float64)))
	if err != nil {
		return res.ErrorResponse(err).Send(c)
	}

	return res.CustomSuccessBuilder(200, result.Datas, "Get user carbon producer success", &result.PaginationInfo).Send(c)
}